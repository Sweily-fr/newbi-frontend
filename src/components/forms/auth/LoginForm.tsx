...
export const LoginForm = ({ onSwitchToRegister }: LoginFormProps) => {
  ...
  const [login] = useMutation(LOGIN_MUTATION, {
    ...
    onError: (error) => {
      ...
      if (errorCode === 'ACCOUNT_DISABLED' || errorMsg.includes('compte a été désactivé')) {
      ...
      } else if (errorMsg.includes('mot de passe') || errorMsg.includes('password')) {
  ...
  const onSubmit: SubmitHandler<LoginFormValues> = async (data) => {
    try {
      // Réinitialiser l'erreur de vérification d'email
      setEmailVerificationError(null);
      // Stocker l'email pour pouvoir le réutiliser si besoin de renvoyer l'email de vérification
      ...
      await login({
        variables: {
          input: {
            ...
            rememberMe: data.rememberMe // Transmettre cette information au serveur
          },
        },
      });
      ...
      if (data.rememberMe) {
        // Sauvegarder l'email et le fait que l'option est activée
      ...
      // Les erreurs sont déjà gérées par onError du useMutation
      // Nous ne faisons que les logger ici pour le débogage
  ...
  useEffect(() => {
    if (message) {
      ...
      if (message.toLowerCase().includes('vérifier') && message.toLowerCase().includes('email')) {
  ...
  return (
    <div className="py-6 px-4 w-full mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-lg">Bienvenue, heureux de te voir de nouveau !</p>
      </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-blue-700">
                Un email de vérification a été envoyé à votre adresse email. Veuillez vérifier votre boîte de réception et cliquer sur le lien de vérification avant de vous connecter.
              </p>
            </div>
          </div>
        </div>
      ...
      {emailVerificationError && (
        <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-md mb-6 max-w-lg mx-auto">
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-amber-800">Vérification requise</h3>
                ...
                <div className="-mx-2 -my-1.5 flex">
                  <button
                    onClick={() => {
                      ...
                      navigate('/resend-verification', { state: { email: lastAttemptedEmail } });
                    }}
      ...
      <Form onSubmit={handleSubmit(onSubmit)} spacing="normal" className="max-w-lg mx-auto">
            <TextField
              id="email"
              label="Email"
              type="email"
