        ...
        if (isNewClient) {
          ...
          if (!selectedClientData) {
            ...
            if (clientsData.clients.length > 0) {
              return {
                id: clientsData.clients[0].id,
                type: clientsData.clients[0].type, // Utiliser le type exact du client sans valeur par défaut
                name: clientsData.clients[0].name,
                email: clientsData.clients[0].email,
                address: {
                  street: clientsData.clients[0].address.street,
                  city: clientsData.clients[0].address.city,
                  postalCode: clientsData.clients[0].address.postalCode,
                  country: clientsData.clients[0].address.country,
                },
                // Champs spécifiques aux entreprises
                siret: clientsData.clients[0].siret || "",
                vatNumber: clientsData.clients[0].vatNumber || "",
                // Champs spécifiques aux particuliers
                firstName: clientsData.clients[0].firstName || "",
                lastName: clientsData.clients[0].lastName || ""
              };
            }
          }
