...
import { AddCircle, Building, SearchNormal1 } from "iconsax-react";
import {
  EMAIL_PATTERN,
  SIRET_PATTERN,
  VAT_PATTERN,
...
export const ClientSelection: React.FC<ClientSelectionProps> = ({
  ...
  const fillFormWithCompanyData = React.useCallback(
    (company: CompanySearchResult) => {
      ...
      if (postalCityMatch) {
        ...
        if (altFormatMatch) {
          ...
          const cityInAddressMatch = streetValue.match(
            /\b(\d{5})\s+([^,\d]+)\b/
          );
  ...
  useEffect(() => {
    if (isNewClient) {
      ...
      if (
        !newClient.type ||
        newClient.type === ("" as unknown as ClientType) ||
  ...
  useEffect(() => {
    ...
    isNewClient,
    setSelectedClient,
  ]); // Exécuter l'effet lorsque invoice, quote, selectedClient, selectedClientData ou isNewClient change
  ...
  const clientOptions = React.useMemo(() => {
    ...
    if (
      selectedClient &&
      !options.some((option) => option.value === selectedClient)
    ) {
      ...
      // Si le client sélectionné n'est toujours pas dans les options, chercher dans clientsData
        ...
        const client = clientsData.clients.items.find(
    ...
    <div className="mb-8 space-y-6">
          ...
          tabs={[
            {
              ...
              icon: <Building size="20" color="#5b50ff" />,
            },
            {
              id: "new",
              label: "Nouveau client",
              icon: <AddCircle size="20" color="#5b50ff" />,
            },
          ]}
          ...
          onTabChange={(tabId) => {
            ...
            if (
              ...
              if (invoice?.client?.id) {
                ...
                const invoiceClientOption = clientOptions.find(
                  (option) => option.value === invoice.client?.id
                );
                ...
                const quoteClientOption = clientOptions.find(
                  (option) => option.value === quote.client?.id
                );
      ...
      {!isNewClient ? (
        <div>
          ...
          {selectedClientData && (
            <div className="mt-4 p-4 bg-[#f0eeff] rounded-2xl shadow-sm border border-[#e6e1ff]">
              <h3 className="text-sm font-medium text-[#5b50ff] mb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedClientData.address?.street},{" "}
                    {selectedClientData.address?.postalCode}{" "}
                    {selectedClientData.address?.city},{" "}
                    {selectedClientData.address?.country}
                  </p>
                </div>
                ...
                {selectedClientData.vatNumber && (
                  <div>
                    <p className="text-xs text-gray-600">Numéro de TVA</p>
                    <p className="text-sm">{selectedClientData.vatNumber}</p>
                  </div>
                )}
                {selectedClientData.hasDifferentShippingAddress &&
                  selectedClientData.shippingAddress && (
                    <div className="col-span-1 md:col-span-2 mt-2 p-3 bg-white rounded-md shadow-sm border border-[#e6e1ff]">
                      <p className="text-xs font-medium text-[#5b50ff] mb-1">
                        Adresse de livraison
                      </p>
                      <p className="text-sm">
                        {selectedClientData.shippingAddress.street},{" "}
                        {selectedClientData.shippingAddress.postalCode}{" "}
                        {selectedClientData.shippingAddress.city},{" "}
                        {selectedClientData.shippingAddress.country}
                      </p>
                    </div>
                  )}
              </div>
        ...
        <FieldGroup spacing="normal" className="space-y-8">
          ...
          {newClient.type === ClientType.COMPANY && !showManualEntry && (
            <div className="mb-6">
                ...
                <div className="flex space-x-2 mt-8">
                  ...
                  <div className="flex items-end pb-2 mb-4">
                    <Button
                      ...
                      className="h-[42px] bg-[#5b50ff] hover:bg-[#4a41e0] text-white border-transparent hover:border-transparent transition-colors duration-200"
                    >
                      <SearchNormal1 size="20" color="#5b50ff" />
                    </Button>
                  </div>
                </div>
                  ...
                  >
                    <ul className="py-1 max-h-[70vh] overflow-y-auto">
                      {companiesList.map((company, index) => (
                        <li
                          key={index}
                          className="px-4 py-3 hover:bg-[#f0eeff] cursor-pointer transition-colors duration-150 border-b border-[#e6e1ff] last:border-b-0"
                          onClick={() => handleSelectCompany(company)}
                        >
                          <div className="font-medium text-[#5b50ff]">
                            {company.name}
                          </div>
              ...
              <button
                type="button"
                className="mt-4 inline-flex items-center text-sm text-[#5b50ff] hover:text-[#4a41e0] transition-colors duration-200"
                onClick={() => setShowManualEntry(true)}
              >
                <AddCircle size="20" color="#5b50ff" className="mr-1" />
          ...
          {(showManualEntry || newClient.type === ClientType.INDIVIDUAL) && (
            <div className="pt-8">
              ...
              {newClient.type === ClientType.COMPANY && (
                <Button
                  variant="secondary"
                  className="border mb-8 text-[#5b50ff] hover:bg-[#f0eeff] transition-colors duration-200"
                  onClick={() => setShowManualEntry(false)}
                >
                  <SearchNormal1 size="20" color="#5b50ff" className="mr-2" />
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      if (isNewClient && newValue.trim() === "") {
                        ...
                        newValue &&
                        !/^[A-Za-zÀ-ÖØ-öø-ÿ\s\-']{2,50}$/.test(newValue)
                      ) {
                        setValidationErrors((prev) => ({
                          ...prev,
                          firstName:
                            "Le prénom doit contenir entre 2 et 50 caractères et ne peut contenir que des lettres, espaces, tirets ou apostrophes",
                        }));
                      } else {
                        setValidationErrors((prev) => {
                    ...
                    required={
                      isNewClient && newClient.type === ClientType.INDIVIDUAL
                    }
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    if (
                      ...
                      newValue &&
                      !/^[A-Za-zÀ-ÖØ-öø-ÿ0-9\s\-'&.]{2,50}$/.test(newValue)
                    ) {
                      setValidationErrors((prev) => ({
                        ...prev,
              ...
              {newClient.type === ClientType.COMPANY && (
                <div className="grid grid-cols-2 gap-8 mb-8">
                  <TextField
                    ...
                    required={
                      isNewClient && newClient.type === ClientType.COMPANY
                    }
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      if (isNewClient && newValue.trim() === "") {
                        ...
                        newValue &&
                        !POSTAL_CODE_PATTERN.test(newValue)
                      ) {
                        setValidationErrors((prev) => ({
                          ...prev,
              ...
              {newClient.hasDifferentShippingAddress && (
                <FieldGroup
                  spacing="normal"
                  className="mt-8 p-8 bg-[#f0eeff] rounded-2xl shadow-sm border border-[#e6e1ff]"
                >
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        ...
                        if (
                          ...
                          setValidationErrors((prev) => {
                            // eslint-disable-next-line @typescript-eslint/no-unused-vars
                            const { "shipping-postalCode": unused, ...rest } =
                              prev;
                            return rest;
                          });
                        }
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        ...
                        if (
                          ...
                          setValidationErrors((prev) => {
                            // eslint-disable-next-line @typescript-eslint/no-unused-vars
                            const { "shipping-country": unused, ...rest } =
                              prev;
                            return rest;
                          });
                        }
