...
export const SocialLinksComponent: React.FC<SocialLinksProps> = ({
  ...
  // Couleur pour le SVG des icônes
  // Utiliser la couleur d'icône fournie si elle existe, sinon utiliser blanc (#fff) comme couleur par défaut
  // Cette approche permet de conserver la fonctionnalité de changement de couleur tout en ayant une couleur par défaut optimale
  const defaultColor = socialLinksIconStyle === 'plain' ? '#333333' : '#ffffff';
  // Utiliser la couleur d'icône fournie si elle existe, sinon utiliser la couleur par défaut
  const svgIconColor = propIconColor || defaultColor;
  
  // Afficher la couleur de l'icône dans la console pour le débogage
  console.log('SocialLinks - iconColor:', propIconColor, 'svgIconColor:', svgIconColor, 'socialLinksIconStyle:', socialLinksIconStyle);
  
  useEffect(() => {
    console.log('SocialLinks - La couleur a changé:', svgIconColor);
  }, [svgIconColor]);
  
  // Définir un style SVG qui applique la couleur directement
  const svgStyle = {
    fill: svgIconColor,
    color: svgIconColor,
    stroke: 'none'
  ...
  // Cette couleur est indépendante de la couleur des icônes SVG
  ...
  const socialLinksContainerStyle: React.CSSProperties = {
    ...
    marginTop: '0', // Suppression de la marge supérieure pour laisser le parent gérer l'espacement
  ...
  const getLinkStyle = (linkType: string): SocialLinkStyle => {
    ...
    if (displayMode === 'text') {
      return {
        ...
        margin: '0',
        padding: '4px 8px',
        color: isHovered ? '#4a41e0' : svgIconColor, // Utiliser la couleur des icônes pour les liens en mode texte
        textDecoration: 'none',
    ...
    switch (socialLinksIconStyle) {
      case 'rounded':
        return {
          ...
          color: svgIconColor, // Utiliser la couleur d'icône spécifiée pour le SVG
      ...
      default:
        return {
          ...baseStyle,
          color: isHovered ? '#4a41e0' : svgIconColor, // Utiliser la couleur d'icône spécifiée pour le SVG
          iconStyle: {
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
    ...
    <div style={socialLinksContainerStyle}>
        ...
        >
          {displayMode === 'icons' ? (
            <div style={getLinkStyle('linkedin').iconStyle || {}}>
              <svg width="14" height="14" viewBox="0 0 24 24" style={svgStyle}>
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
            </div>
          ) : <p style={{ color: svgIconColor }}>Linkedin</p>}
        </a>
      )}
      {socialLinks?.twitter && (
        <a 
        ...
        >
          ...
          ) : <p style={{ color: svgIconColor }}>Twitter</p>}
        </a>
      )}
      {socialLinks?.facebook && (
        ...
        >
          ...
          ) : <p style={{ color: svgIconColor }}>Facebook</p>}
        </a>
      )}
      {socialLinks?.instagram && (
        ...
        >
          ...
          ) : <p style={{ color: svgIconColor }}>Instagram</p>}
        </a>
      )}
    </div>
  );
